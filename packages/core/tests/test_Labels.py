...
class TestSaveLabels:
    ...
    @patch("metahq_core.curations.labels.Labels.save")
    def test_save(self, mock_write, labels_instance, tmp_path, fmt):
        """Test saving to various file types."""
        file_path = tmp_path / f"test.{fmt}"

        labels_instance.save(file_path, fmt=fmt)

        mock_write.assert_called_once_with(file_path, fmt=fmt)
...
class TestLabelsEdgeCases:
    ...
    def test_single_row(self):
        ...
        ids = pl.DataFrame(
            {"index": ["sample_1"], "group": ["study_a"], "platform": ["rna"]}
        )
