"""
Command-line interface for metahq-build.
...
@click.group()
@click.version_option(version=__version__, prog_name="metahq-build")
@click.pass_context
def main(ctx):
    """
...
def package(config, data_dir, output_dir, start_from, end_at, verbose):
    ...
    try:
        if config is None:
            click.secho(
                "Error: --config is required. Pass the path to metahq_build.yaml.",
                fg="red",
            )
            sys.exit(1)
        ...
        from metahq_build.pipeline import PipelineOrchestrator
...
def process(source_name, output_dir, no_validate):
    ...
    Examples:
        ...
        metahq-build process disign_atlas --output-dir /custom/output
    """
    try:
        # Get processor
...
def download_gemma(output, query, max_studies):
    ...
    """
    from metahq_build.config.config import GEMMA_RAW
...
def combine_geo(output):
    ...
    """
    from metahq_build.combiners.geo import GeoCombiner
    from metahq_build.config import GEO_COMBINED_BSON
...
def combine_sra(output, metadata_db):
    ...
    """
    from metahq_build.combiners.sra import SraCombiner
    from metahq_build.config.config import OMICIDX_DB, SRA_COMBINED_BSON
...
def combine_sample(output, geo, sra, metadata_db, specific):
    ...
    'metahq-build combine geo' and 'metahq-build combine sra' first.
...
def retrieve_sample_metadata(fields, outfile, sample_bson, metadata_db, null_values):
    ...
    from metahq_build.config.config import OMICIDX_DB, SAMPLE_COMBINED_BSON
...
def retrieve_series_metadata(fields, outfile, series_bson, metadata_db, null_values):
    ...
    from metahq_build.config import OMICIDX_DB, SERIES_COMBINED_BSON
...
def init_config(output_path, data_dir, output_dir):
    ...
    Examples:
        ...
        metahq-build init-config config.yaml --data-dir /data/metahq
    """
    try:
...
def ontology_search_db(mondo, uberon_cl, out_db):
    ...
    from metahq_build.builders import OntologySearchDbBuilder
    from metahq_build.config import (
